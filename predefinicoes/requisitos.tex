...
\newcommand{\AppendRequisitoFunc}[2]{%
    \immediate\write\reqFunc{#1 & \expandafter#2 \unexpanded{\\}}%
}
%
...
%% sua expansão tem que ser postergada para o momento em
%% que a tabela está sendo impressa (`PrintVolunteers`).
\newcommand{\AppendRequisitoNFunc}[2]{%
    \immediate\write\reqNFunc{#1 & \expandafter#2 \unexpanded{\\}}%
}
%
...
\newcommand{\ReqTipo}[1]{%
    \ifstrequal{#1}{funcional}{%
        ...
        \AppendRequisitoFunc{\CmdReqLabel}{\CmdReqNome}
    }{%
        \stepcounter{indiceNaoFuncional}%
        \ReqLabel{RNF\theindiceNaoFuncional}%
        \renewcommand{\CmdReqTipo}{\multicolumn{3}{|l|}{\textbf{No:} RNF\theindiceNaoFuncional} & ( ) Funcional & (X) Não Funcional \\}
        \AppendRequisitoNFunc{\CmdReqLabel}{\CmdReqNome}
    }
}
